...
power stamp or evaluating any PSF branch. It also retains the existing PSF-model
NaN check and scans all 24 output fields after later calculations. Every invalid
case reaches the same final writer and emits exactly 24 `-99999.0` values. Stage
9 consumes the paired shear/original rows and uses its historical `cat[0] < -900`
cut to omit sentinel sources from the combined science catalog. No new runtime or
compile-time sentinel configuration was added.
