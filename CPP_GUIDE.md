...
no PCA. It is also fixed to historical Type-1 preprocessing, no Stage-3
background re-subtraction, deterministic F77 blank-noise stamps, F77
largest-component PSF grouping, and one initial full fit without PRESS star
rejection/refitting. Because C++ Lite fixes the per-chip DQ branch, its runs
require DQ masks; C++ Standard can select a configuration that does not read them.
...
- `PathConfig::ASTROMETRY_TILE_PREFIX` and
  ...
  value. Changing a prefix intentionally does not recognize legacy names.
...
| Gaia catalog layout | `AstroCatType=1`, `ASTROMETRY_TILE_PREFIX="astra_"` | type in `config/LensingConfig.hpp`; prefix in `config/pathconfig.hpp`, compile-time | `1` reads legacy large `gaia_*.cat` tiles; `2` accumulates the one-degree `<prefix>RA_*.dat` tiles produced by `process_astrocat`. Set `ASTROMETRY_CAT` separately to the directory consumed by Stage 1 and rebuild after changing the type or prefix. |
| External-catalog discovery and parsing | `EXTCAT_INPUT_DIRECTORY`, `EXTCAT_OUTPUT_DIRECTORY`, `SOURCE_CAT_TILE_PREFIX="extern_"` | paths/prefix in `config/pathconfig.hpp`; parsing settings in `config/ExtCatConfig.hpp`; runtime `--extcat-input`, `--extcat-output` | Change the external-catalog file organization. The output directory cannot equal or sit below the input directory. The prefix is compile-time and shared by `process_extcat` and SOURCE_CAT lookup. |
...
| Standard compatibility selectors | `PreprocsType=1`, `include_BGsub=0`, `NstampType=1`, `PsfGroupingType=1` | `config/LensingConfig.hpp`, compile-time | Current Standard defaults select historical preprocessing, no Stage-3 background re-subtraction, deterministic F77 blank noise, and F77 largest-component grouping. Standard still retains the alternate branches. Lite has no corresponding selectors and physically implements only these frozen paths. |
...
| FD detector rules | `bad_ccds={}`, `chip_mask_edge=50` | `config/FDConfig.hpp`, compile-time | Bounds are derived as inclusive `[edge, chipnx-edge]` and `[edge, chipny-edge]`; change the edge or bad-CCD list for another camera and keep `n_bad_ccds` synchronized. |
...
full-fit diagnostics in both Standard and Lite. Standard may use leave-one-out
values inside optional PRESS rejection on modern grouping paths. Lite never performs
PRESS rejection; it computes leverage/analytic LOO only to validate its initial-fit
cache, and those values are not serialized into this product.
