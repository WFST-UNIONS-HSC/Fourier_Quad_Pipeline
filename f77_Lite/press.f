...
c ==========================================
c Function: Initialize a distinct ran1 stream for one MPI rank
c Method: Pack clock/rank and apply a bijective fifth-power map
c ==========================================
      SUBROUTINE initialize_ran1_seed(my_id,num_procs,seed)
      IMPLICIT NONE

      INTEGER my_id,num_procs,seed,j
      INTEGER idum,iv(32),iy
      COMMON /ran1_state/ idum,iv,iy
      SAVE /ran1_state/

      INTEGER*8 clock_count,prime,limit,nproc8,slots
      INTEGER*8 packed,power2,power4,seed8
      PARAMETER (prime=2147483647_8)

      if (num_procs.le.0) stop 'Invalid MPI process count for ran1'
      if (my_id.lt.0 .or. my_id.ge.num_procs)
     .  stop 'Invalid MPI rank for ran1'

      limit=prime-1_8
      nproc8=int(num_procs,8)
      if (nproc8.gt.limit) stop 'Too many MPI ranks for ran1'

      slots=limit/nproc8
      call system_clock(clock_count)

c Encode rank in the residue modulo num_procs, so ranks cannot collide.
      packed=modulo(clock_count,slots)*nproc8+int(my_id,8)+1_8

c The fifth-power map is one-to-one because gcd(5,prime-1)=1.
      power2=modulo(packed*packed,prime)
      power4=modulo(power2*power2,prime)
      seed8=modulo(power4*packed,prime)

      if (seed8.le.0_8 .or. seed8.ge.prime)
     .  stop 'Generated ran1 seed is outside the valid range'
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c ==========================================
c Function: Generate the next ran1 uniform deviate
c Method: Use the original shuffled Park-Miller recurrence
c ==========================================
      FUNCTION ran1()
...
c ==========================================
c Function: Define the fallback ran1 state
c Method: Preserve seed -123 for standalone callers without MPI seeding
c ==========================================
      BLOCK DATA initialize_ran1_state
      IMPLICIT NONE

      INTEGER idum,iv(32),iy
      COMMON /ran1_state/ idum,iv,iy
      DATA idum /-123/,iv /32*0/,iy /0/
