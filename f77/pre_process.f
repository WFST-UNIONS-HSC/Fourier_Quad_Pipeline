...
c      subroutine chip_pre_process(IMAGE_FILE,DIR_OUTPUT)
      ...
      ! call readimage(MASK_FILE,nx,ny,npx,npy,flat)
...
c     Super flat field correction
        if (include_Mask.eq.1 .or. include_Mask.eq.3) then
          call readimage(MASK_FILE,nxx,nyy,npx,npy,flat_weight)
...
c divide the chip into nbx*nby blocks, each with sidelength of
c "blocksize":
...
c The purpose of this code is to achieve a rough flattening of the
c background.
...
c The purpose of this code is to mark the parts affected by stellar
c halos.
